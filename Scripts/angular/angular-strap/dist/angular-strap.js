	  	...
	  	this.$get = ["$window", "$rootScope", "$tooltip", function ($window, $rootScope, $tooltip) {
	  		...
	  		function TypeaheadFactory(element, controller, config) {
	  			...
	  			$typeahead.select = function (index) {
	  				...
	  				if (angular.isDefined(options.onSelect) && angular.isDefined(scope[options.onSelect]) && $typeahead.$isVisible()) {
	  					scope[options.onSelect](value);
	  					show();
	  				}
	  ...
	  .directive('bsTypeahead', ["$window", "$parse", "$q", "$typeahead", "$parseOptions", function ($window, $parse, $q, $typeahead, $parseOptions) {
	  	...
	  	return {
	  		...
	  		link: function postLink(scope, element, attr, controller) {
	  			...
	  			// Directive options
	  			var options = { scope: scope };
	  			angular.forEach(['placement', 'container', 'delay', 'trigger', 'keyboard', 'html', 'animation', 'template', 'filter', 'limit', 'minLength', 'watchOptions', 'selectMode', 'onSelect'], function (key) {
	  				if (angular.isDefined(attr[key])) options[key] = attr[key];
	  			});
